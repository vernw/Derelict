Version 12 of Reactable Quips by Michael Martin begins here.

"A table-based approach to NPC conversation chains, as well as allowing rules to fire on lines of conversation."

[Version 12 - Proper format for the 6L02 patch, which adds adaptive text and responses.]
[Version 11 - Emergency patch for 6L02, which fixes a new compilation problem.]
...
Carry out requesting a recap (this is the perform recap rule):
	...
	repeat through Table of Quip Followups:
		if the quip entry is the pertinent quip:
			increase rq_index by 1;
			if rq_index is 1:
				say "The available options are:" (A);
				say paragraph break;
			say "[bracket][rq_index][close bracket] [option entry][line break]";
	if rq_index is 0, say "No options are currently available." (B).
...
Before doing something [other than requesting a recap or responding with] when the pertinent quip is not quip_null (this is the force conversation rule):
	unless the current action is requesting a recap or the current action is responding with:
		say "Please enter a number, or say REPEAT to list the options again." (A);
		stop the action.
...
To think is a verb.
The generic ask quip is some text that varies. The generic ask quip is usually "[We] [can't think] of anything to say on that topic.".
The generic tell quip is some text that varies. The generic tell quip is usually "[We] [can't think] of anything to say on that topic.".
...
All other messages are treated as library responses and can be accessed through the Index.
